...
window.initUpdateMessageScripts = function () {
...
function initializeDataForUpdate() {
    ...
    const broadcastType = window.updateMessageModel?.broadcastType;
...
function loadSelectedUsers() {
    ...
    if (messageInternsId && messageInternsId.length > 0) {
        fetch('/Interns/GetInternsByIds', {
            ...
            body: JSON.stringify(messageInternsId)
        })
...
window.updateMessageFormSubmitHandler = function (event) {
    ...
    if (value == 'All') {
        ...
        const messageInternsId = (window.updateMessageModel?.internsId || []).map(id => String(id));

        selectedUserIds.forEach((userId, index) => {
            event.detail.parameters.append(`UpdatedInternsId[${index}]`, userId);
        });

        messageInternsId.forEach((userId, index) => {
